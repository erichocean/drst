...
static bool storeScreenshot = false;
...
namespace embree
{
  ...
  case 'p': storeScreenshot = true; return;
...
#define ROTATE_WITH_FIXED_UPVECTOR 1
  ...
  if (storeScreenshot) {
    ...
    pixels.resize(4 * g_width*g_height, 255);

    if (g_format == "RGB_FLOAT32") {
      float* src = reinterpret_cast<float*>(ptr);
      for (unsigned i = 0; i < g_width*g_height; ++i) {
        pixels[i * 4 + 0] = (int)(src[i * 3 + 0] * 255.0f + 0.5f);
        pixels[i * 4 + 1] = (int)(src[i * 3 + 1] * 255.0f + 0.5f);
        pixels[i * 4 + 2] = (int)(src[i * 3 + 2] * 255.0f + 0.5f);
      }
    }
    else if (g_format == "RGBA8") {
      const unsigned char* src = reinterpret_cast<unsigned char*>(ptr);
      for (unsigned i = 0; i < g_width*g_height; ++i) {
    ...
    lodepng::encode("output.png", pixels, g_width, g_height);
    std::cout << "output.png saved (" << g_format << ")" << std::endl;
  }
