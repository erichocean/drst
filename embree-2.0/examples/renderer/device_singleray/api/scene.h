...
#include "embree/geometry/geometry.h"
...
namespace embree
{
  ...
  public:
    ...
    BackendScene (RTCGeometry* accel, RTCIntersector1* intersector1, RTCIntersectorStream* intersectorStream)
      : accel(accel), intersector(intersector1), intersectorStream(intersectorStream) {
    printf("created scene with %.1f M triangles.\n", accel->size()*1e-6f);
  }
    ...
    virtual void postIntersect(const StreamRay& ray, const StreamRayExtra& rayExtra, const StreamHit& hit, DifferentialGeometry& dg) const = 0;
  ...
  public:
    ...
    RTCIntersectorStream* intersectorStream;         //!< Acceleration structure intersector
  };
}
