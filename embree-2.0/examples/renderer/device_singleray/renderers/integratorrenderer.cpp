...
#include "integrators/pathtracestreamintegrator.h"
#include "integrators/raystream.h"
...
namespace embree
{
  static const bool streamTraversal = true;
    ...
    : iteration(0)
  {
    ...
    if (!streamTraversal && _integrator == "pathtracer") integrator = new PathTraceIntegrator(parms);
    else if (streamTraversal && _integrator == "pathtracer") integrator = new PathTraceStreamIntegrator(parms, samplers->samplesPerPixel * TILE_SIZE * TILE_SIZE);
    ...
    accumulate(accumulate), iteration(iteration), tileID(0), atomicNumRays(0)
  {
    ...
    renderer->samplers->init(rcpWidth, rcpHeight, iteration,renderer->filter);
  ...
  void IntegratorRenderer::RenderJob::finish(size_t threadIndex, size_t threadCount, TaskScheduler::Event* event)
  {
    ...
    stream << atomicNumRays*1e-6 << " mrays";
  ...
  void IntegratorRenderer::RenderJob::renderTile(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
  {
    ...
    if (renderer->samplers->sampleSets > 256)
      throw std::runtime_error("too many sample sets");

    unsigned spp = renderer->samplers->samplesPerPixel;

    TilePipeline tilePipeline(renderer->integrator.ptr, 1, spp, renderer->samplers->samples, accumulate, swapchain.ptr, toneMapper.ptr, framebuffer.ptr);
    ...
    while (true)
    {
      ...
      if (streamTraversal) {
        Tile* tileData = tilePipeline.advanceUntilFree();

        tileData->setup(camera.ptr, tile_x, tile_y, randomNumberGenerator, renderer->samplers->sampleSets, rcpWidth, rcpHeight);
        tileData->dipatch(scene.ptr, threadIndex);
      }
      else {
        ...
        for (size_t dy=0; dy<TILE_SIZE; dy++)
        {
          ...
          for (size_t dx=0; dx<TILE_SIZE; dx++)
          {
            ...
            for (size_t s=0; s<spp; s++)
            {
              ...
              const float fx = float(x)*rcpWidth + sample.pixel.x;
              const float fy = float(y)*rcpHeight + sample.pixel.y;
              ...
              L += renderer->integrator->Li(threadIndex, primary, scene, state);
            }
    ...
    tilePipeline.processOutstanding();
    state.numRays += tilePipeline.computeNumRays();
