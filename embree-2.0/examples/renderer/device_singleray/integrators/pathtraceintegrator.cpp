...
namespace embree
{
  ...
  Color PathTraceIntegrator::Li(size_t thread, LightPath& lightPath, const Ref<BackendScene>& scene, IntegratorState& state)
  {
    ...
    if (dg.material) dg.material->shade(lightPath.lastMedium, dg, brdfs);
    ...
    if (lightPath.depth < maxDepth)
    {
      ...
      if (c != Color(zero) && wi.pdf > 0.0f)
      {
        ...
        L += c * Li(thread, scatteredPath, scene, state) * rcp(wi.pdf);
      }
    }
  ...
  Color PathTraceIntegrator::Li(size_t thread, Ray& ray, const Ref<BackendScene>& scene, IntegratorState& state) {
    LightPath path(ray); return Li(thread, path,scene,state);
  }
}
