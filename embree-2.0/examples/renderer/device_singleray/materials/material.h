...
namespace embree
{
  ...
  public:
    ...
    Material (const Medium& mediumOutside = Medium::Vacuum(), /*!< The medium the geometry normal points into. */
      ...
      : mediumOutside(mediumOutside), mediumInside(mediumInside), isMediaInterface(isMediaInterface), computeTextureCoordinates(false), computeTangentBasis(false) {}

    /*! Virtual destructor for materials */
    virtual ~Material() {}

    /*! Shades a location and and returns a composited BRDF */
    virtual void shade(const Medium&               currentMedium,    /*!< The medium this ray travels inside. */
  ...
  bool computeTextureCoordinates;
