...
namespace embree
{
  ...
  struct IntegratorStreamState
  {
    unsigned char* __restrict pixelSampleSets;
    Color* __restrict pixelOutput;
    Vec2f* __restrict pixelLocations;
    const PrecomputedSample* const __restrict* __restrict sampleSets;
    unsigned samplesPerPixelLog2;
    unsigned numRays;
  };
  ...
  public:
    virtual RayStream<false>& getRayStream() = 0;

    virtual void dispatch(Tile* tile, BackendScene* scene, IntegratorStreamState& state, unsigned thread) = 0;

    virtual bool process() = 0;

    virtual ~TileIntegrator() {}
  };
  ...
  public:
     ...
     *  the ray direction. */
    virtual Color Li(size_t thread,
                     Ray&               ray,     /*!< Ray to compute the radiance along.                */
                     const Ref<BackendScene>& scene,   /*!< Scene geometry and lights.                        */
                     IntegratorState&   state) = 0;
    
    virtual TileIntegrator* createTileIntegrator() = 0;
  };
}
