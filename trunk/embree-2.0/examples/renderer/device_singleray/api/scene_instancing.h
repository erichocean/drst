...
namespace embree
{
  ...
  public:
    ...
    public:
      ...
      __forceinline void postIntersect(const StreamRay& ray, const StreamRayExtra& rayExtra, const StreamHit& hit, DifferentialGeometry& dg) const
      {
        dg.material   = material.ptr;
        dg.light      = (AreaLight*)light.ptr;
        dg.illumMask  = illumMask;
        dg.shadowMask = shadowMask;
        shape->postIntersect(ray,rayExtra,hit,dg);
        if (hasTransform) {
          dg.Tx   = xfmVector(local2world,dg.Tx); 
          dg.Ty   = xfmVector(local2world,dg.Ty);
          dg.Ng = xfmVector(normal2world,dg.Ng);
    ...
    public:
      ...
      void create() 
      {
        ...
        RTCIntersectorStream* intersectorStream = rtcQueryIntersectorStream(objs,"default.default");
        
        /* create new scene */
        instance = new BackendSceneInstancing(prims,objs,intersector1,intersectorStream);
      }
    ...
    BackendSceneInstancing (const std::vector<Ref<Primitive> >& geometry, RTCGeometry* accel, RTCIntersector1* intersector1, RTCIntersectorStream* intersectorStream)
      : BackendScene(accel,intersector1,intersectorStream), geometry(geometry)
    {
    ...
    void postIntersect(const StreamRay& ray, const StreamRayExtra& rayExtra, const StreamHit& hit, DifferentialGeometry& dg) const {
      if (hit) geometry[hit.id0]->postIntersect(ray,rayExtra,hit,dg);
    }
