...
namespace embree
{
  ...
  __forceinline Color eval(const Vector3f& wo, const DifferentialGeometry& dg, const Vector3f& wi, BRDFType type) const
    {
      Color c = zero;
  ...
  __forceinline bool has(const BRDFType &type) {
  ...
  __forceinline Color sample(const Vector3f               & wo,          /*!< Direction light is reflected into.                    */
    ...
    {
    if (size() == 1) {
      if (BRDFs[0]->type & type) {
        Sample3f wi; Color c = BRDFs[0]->sample(wo, dg, wi, s);
        wi_o = wi;
        type_o = BRDFs[0]->type;
        return c;
      }
    ...
    ss *= sum;
      ...
      /*! return */
      wi_o = Sample3f(samples[i].value,samples[i].pdf*(f[i]/sum));
      type_o = types[i];
      return colors[i];
    }
