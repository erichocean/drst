...
namespace embree {
    ...
    public:
        ...
        Obj(const Parms &options) {
            ...
            map_d = options.getTexture("map_d");  float d = options.getFloat("d", 1.0f);

            /*! diffuse reflectance texture and coefficient */
            map_Kd = options.getTexture("map_Kd");  Kd = Color4(options.getColor("Kd", one) * d);
      Kd.a = d;
      ...
      if (map_Kd)
        computeTextureCoordinates = true;
        }

        void shade(const Medium &currentMedium, const DifferentialGeometry &dg, CompositedBRDF &brdfs) const 
        {
          /*if (this->map_Bump) {
      ...
      Color4 Kd = this->Kd;
          ...
          /*! diffuse component */
      if (map_Kd) {
        Kd = _mm_mul_ps(Kd, map_Kd->get(dg.st));
      }

      if (Kd.a < 1.0f) brdfs.add(NEW_BRDF(Transmission)(Color(1.0f - Kd.a)));
      if (Kd != Color(zero)) brdfs.add(NEW_BRDF(Lambertian)(Kd));
          
          /*! specular exponent */
          //float Ns = this->Ns;  if (map_Ns) Ns *= map_Ns->get(dg.st).r;
          
          /*! specular component */
          //Color Ks = Kd.a*this->Ks;  if (map_Ks) Ks *= map_Ks->get(dg.st);  if (Ks != Color(zero)) brdfs.add(NEW_BRDF(Specular)(Ks, Ns));
        }

    protected:
        Ref<Texture> map_d;   //float d;     /*! opacity: 0 (transparent), 1 (opaque)                */
        Ref<Texture> map_Kd;  Color4 Kd;    /*! diffuse  reflectance: 0 (none), 1 (full)            */
