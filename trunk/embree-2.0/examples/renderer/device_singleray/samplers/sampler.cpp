...
namespace embree
{
    ...
    if (samples) {
      for (int set = 0; set < sampleSets; set++) {
        /*for (int s = 0; s < samplesPerPixel; s++) {
          alignedFree(samples[set][s].samples1D);
          alignedFree(samples[set][s].samples2D);
  ...
  if (allSamples1D) alignedFree(allSamples1D);
  if (allSamples2D) alignedFree(allSamples2D);
  if (allLightSamples) delete[] allLightSamples;
