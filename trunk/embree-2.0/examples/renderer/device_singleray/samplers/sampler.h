...
namespace embree
{
  ...
  public:
    ...
    __forceinline void operator = (const LightSample& rhs) {
      a = _mm_insert_ps(rhs.wi.value, _mm_set1_ps(rhs.wi.pdf), 3 << 4);
      b = _mm_insert_ps(rhs.L, _mm_set1_ps(rhs.tMax), 3 << 4);
    }

    __forceinline operator LightSample () const {
      LightSample ls;
      ls.wi.value = a;
      ls.wi.pdf = a.m128_f32[3];
      ls.tMax = b.m128_f32[3];
      ls.L = Color(b);
      return ls;
    }
  };
  ...
  __forceinline LightSample getLightSample(int lightSampleId) const { return lightSamples[lightSampleId]; }
  ...
  PackedLightSample* lightSamples; //!< Precomputed light samples.
  ...
  public:
    ...
    void init(float rcpWidth, float rcpHeight, int iteration = 0, const Ref<Filter> filter = NULL);
  ...
  float* allSamples1D;
  Vec2f* allSamples2D;
  PackedLightSample* allLightSamples;
  };
}
