...
namespace embree
{
  ...
  const char* const IntersectorStream::name = "IntersectorStream";
  ...
  RTCGeometry* rtcNewVirtualGeometry (const size_t numObjects, const char* accelTy) {
    return 0;//new VirtualScene(numObjects,accelTy);
    // TODO: Virtual scenes unsupported at the moment. /RB
  }
  ...
  IntersectorStream* rtcQueryIntersectorStream (const RTCGeometry* geom, const char* type) {
    return geom->intersectorStream(type);
  }
  
  void rtcDeleteIntersectorStream (IntersectorStream* intersector) {
    delete intersector;
  }
